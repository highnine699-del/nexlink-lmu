...
#   4. Then run:  .\LMU-AutoConnect-GUI.ps1
...
$script:InstanceMutex = New-Object System.Threading.Mutex($false, "Global\LMU-AutoConnect-SingleInstance")
try {
...
if (-not $acquired) {
    ...
    [System.Windows.Forms.MessageBox]::Show("LMU Auto-Connect is already running (check your system tray).", "LMU Auto-Connect") | Out-Null
    exit
}
...
function Save-PortalCredential {
    ...
    if ($result -eq "OK" -and $txtUser.Text -and $txtPass.Text) {
        ...
        try {
            ...
            [System.Windows.Forms.MessageBox]::Show("Unable to save credentials: $($_.Exception.Message)", "LMU Auto-Connect") | Out-Null
            return $false
        }
    }
...
function Get-PortalCredential {
    if (-not (Test-Path $CredFile)) {
        ...
        if (-not (Save-PortalCredential)) {
            [System.Windows.Forms.MessageBox]::Show("No credentials entered. Exiting.", "LMU Auto-Connect") | Out-Null
            exit
        }
    }
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show("Credential file is unreadable or corrupt. Delete $CredFile and run again.", "LMU Auto-Connect") | Out-Null
        exit
    }

    if (-not $data.Username -or -not $data.Password) {
        Add-Log "Credential file is missing username or password field."
        [System.Windows.Forms.MessageBox]::Show("Credential file is missing required values.", "LMU Auto-Connect") | Out-Null
        exit
    }
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show("Saved password could not be decrypted. Delete $CredFile and create it again.", "LMU Auto-Connect") | Out-Null
        exit
    }
...
$form.Text = "LMU Auto-Connect Monitor"
...
function Set-Status($text, $color) {
    ...
    $trayIcon.Text = "LMU Auto-Connect: $text"
}
...
$trayIcon.Text = "LMU Auto-Connect Monitor"
...
$form.Add_Resize({
        if ($form.WindowState -eq "Minimized") {
            $form.Hide()
            $trayIcon.ShowBalloonTip(1500, "LMU Auto-Connect", "Still running in the background.", [System.Windows.Forms.ToolTipIcon]::Info)
        }
    })
...
$form.Add_FormClosing({
        ...
        if (-not $script:allowExit) {
            ...
            $form.Hide()
            $trayIcon.ShowBalloonTip(1500, "LMU Auto-Connect", "Minimized to tray. Right-click the tray icon to exit.", [System.Windows.Forms.ToolTipIcon]::Info)
        }
        else {
